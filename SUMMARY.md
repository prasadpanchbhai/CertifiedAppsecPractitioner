# Table of contents

* [Certified Appsec Practitioner (CAP)](README.md)
* [Input Validation Mechanisms](input-validation-mechanisms.md)
* [Whitelisting & Blacklisting](whitelisting-and-blacklisting.md)
* [Authentication related Vulnerabilities](authentication-related-vulnerabilities.md)
* [Brute force Attacks](brute-force-attacks.md)
* [Password Storage and Password Policy](password-storage-and-password-policy.md)
* [Cross-Site Scripting](cross-site-scripting.md)
* [SQL Injection](sql-injection.md)
* [XML External Entity attack](xml-external-entity-attack.md)
* [Cross-Site Request Forgery](cross-site-request-forgery.md)
* [Encoding, Encryption and Hashing](encoding-encryption-and-hashing.md)
* [Understanding of OWASP Top 10 Vulnerabilities](understanding-of-owasp-top-10-vulnerabilities.md)
* [Same Origin Policy](same-origin-policy.md)
* [Security Headers.](security-headers..md)
* [TLS security](tls-security.md)
* [TLS Certificate Misconfiguration](tls-certificate-misconfiguration.md)
* [Symmetric and Asymmetric Ciphers](symmetric-and-asymmetric-ciphers.md)
* [Server-Side Request Forgery](server-side-request-forgery.md)
* [Authorization and Session Management related flaws](authorization-and-session-management-related-flaws.md)
* [Insecure Direct Object Reference (IDOR)](insecure-direct-object-reference-idor.md)
* [Privilege Escalation](privilege-escalation.md)
* [Parameter Manipulation attacks](parameter-manipulation-attacks.md)
* [Securing Cookies.](securing-cookies..md)
* [Insecure File Uploads](insecure-file-uploads.md)
* [Code Injection Vulnerabilities](code-injection-vulnerabilities.md)
* [Business Logic Flaws](business-logic-flaws.md)
* [Directory Traversal Vulnerabilities](directory-traversal-vulnerabilities.md)
* [Security Misconfigurations.](security-misconfigurations..md)
* [Information Disclosure.](information-disclosure..md)
* [Vulnerable and Outdated Components.](vulnerable-and-outdated-components..md)
* [Common Supply Chain Attacks and Prevention Methods.](common-supply-chain-attacks-and-prevention-methods..md)
